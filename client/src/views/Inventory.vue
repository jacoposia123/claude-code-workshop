...
export default {
  ...
  setup() {
    ...
    // Escape a single CSV field: wrap in quotes and double up internal quotes
    ...
    // mirroring the table's translated/formatted values and column order.
    const exportCsv = () => {
      ...
      const rows = filteredItems.value.map((item) => [
        ...
        item.reorder_point,
        `${currencySymbol.value}${item.unit_cost.toFixed(2)}`,
        `${currencySymbol.value}${(item.quantity_on_hand * item.unit_cost).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
